...
void run_simulation(int a, int b, int x, int N, double p, int P, double *prob_b, double *avg_time, double *elapsed_time) {
    ...
    #pragma omp parallel for num_threads(P) reduction(+:hits_b, total_steps, total_time)
    for (int i = 0; i < N; i++) {
        int x_pos = x;
        struct drand48_data state;
        long seed = time(NULL) ^ omp_get_thread_num() ^ i;  
...
int main(int argc, char *argv[]) {
    ...
    int test_N[] = {100, 1000, 10000, 10000, 100000, 1000000};
	...
	printf("Testing with varying N, fixed P = %d\n", P);
