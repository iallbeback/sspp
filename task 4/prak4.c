...
void benchmark(int N) {
    double *A, *B, *B_T, *B_T_d, *C_seq, *C_avx, *C_avx_d;
	...
	if(N==8) {
		print_matrices_and_diff_to_file(C_seq, C_avx, N, "matrices_and_diff.txt");
		print_matrices_and_diff_to_file_d(C_seq, C_avx_d, N, "matrices_and_diff.txt");
	}
	
	if(are_matrices_equal(C_seq, C_avx, N) && are_matrices_equal(C_seq, C_avx_d, N)){
		printf("equal\n");
	}
	else {
		printf("not equal!!!\n");
